...
import { UniversalBus } from './UniversalBus';
import { Environment } from './Environment';
import { IBus, BusConfig, HTOSMessageTypes } from './types';

// Global bus instance
let _globalBus: UniversalBus | null = null;

/**
 * Create or get global bus instance
 */
export function createBus(config?: Partial<BusConfig>): IBus {
  if (_globalBus) {
    return _globalBus;
  }

  const defaultConfig: BusConfig = {
    appName: 'htos',
    version: '1.0.0',
 ...
 */
export async function initBus(config?: Partial<BusConfig>): Promise<IBus> {
 ...
 */
export function destroyBus(): void {
  if (_globalBus) {
    _globalBus.destroy();
    _globalBus = null;
  }
...
export { UniversalBus } from './UniversalBus';

// HTOS-specific convenience functions
export class HTOSBus {
  private static bus: IBus | null = null;

  /**
   * Initialize HTOS bus with default configuration
   */
  static async init(): Promise<IBus> {
   ...
   */
  private static async setupHTOSHandlers(): Promise<void> {
    if (!this.bus) return;
   ...
   */
  private static async setupBackgroundHandlers(): Promise<void> {
    if (!this.bus) return;

    // Token management events
    this.bus.on(HTOSMessageTypes.TOKEN_EXTRACTED, (provider: string, domain: string) => {
      console.log(`[HTOS Bus] Token extracted for ${provider} from ${domain}`);
    });

    // DNR activation events
    this.bus.on(HTOSMessageTypes.DNR_ACTIVATE, async (provider: string, tabId: number) => {
   ...
   */
  private static async setupContentScriptHandlers(): Promise<void> {
    if (!this.bus) return;

    // UI update handlers
    this.bus.on(HTOSMessageTypes.UI_UPDATE, (update: any) => {
      console.log('[HTOS Bus] UI update received:', update);
    });

    // Prompt dispatch handlers
    this.bus.on(HTOSMessageTypes.PROMPT_DISPATCH, async (prompt: any) => {
   ...
   */
  private static async setupOffscreenHandlers(): Promise<void> {
    if (!this.bus) return;
    ...
    // Iframe management
    this.bus.on(HTOSMessageTypes.OFFSCREEN_IFRAME, (action: string, data: any) => {
   ...
   */
  private static async setupPopupHandlers(): Promise<void> {
    if (!this.bus) return;

    // UI state changes
    this.bus.on(HTOSMessageTypes.UI_STATE_CHANGE, (state: any) => {
      console.log('[HTOS Bus] UI state change:', state);
    });

    // Notifications
    this.bus.on(HTOSMessageTypes.UI_NOTIFICATION, (notification: any) => {
   ...
   */
  static async sendSystemMessage(type: string, ...args: any[]): Promise<any> {
    const bus = this.get();
   ...
   */
  static async broadcastSystemEvent(type: string, ...args: any[]): Promise<void> {
    const bus = this.get();
