...
(() => {
  ...
  htosApp.$ai = {
    // Complete OpenAI Arkose Configuration (from HARPA)
    arkose: {
      configs: {
        'chatgpt.com': {
          ...
          // Requirements endpoint for getting DX token
          requirements: {
          ...
          script: {
            src: null, // Dynamically set
            'data-status': 'loading',
            'data-callback': 'useArkoseSetupEnforcement'
          },
          
          // Arkose SDK parameters
          params: {
            mode: 'inline'
          },
          
          // Complete Arkose parameters for OpenAI
          parameters: {
            capi_mode: 'lightbox',
            capi_version: '1.5.2',
            capi_settings: null,
            public_key: '35536E1E-65B4-4D96-9D97-6ADB7EFF8147', // OpenAI's Arkose public key!
          ...
          // Enforcement configuration for iframe
          enforcement: {
            efKey: 'ef',
            sent: 'sent',
            isSDK: 'isSDK',
      ...
      // Token fetch promise for async handling
      _fetchTokenPromise: null as any,
      _arkose: null as any,
    },
    ...
    _loadArkoseScript: async (config: any) => {
      try {
        ...
        // Set the real Arkose SDK URL for OpenAI
        script.src = `${config.iframeUrl}fc/api/nojs/?pkey=${config.parameters.public_key}`;
        
        document.head.appendChild(script);
        
        // Wait for script to load with config timeout
        await new Promise((resolve, reject) => {
          script.onload = resolve;
          script.onerror = reject;
          setTimeout(() => reject(new Error('SDK load timeout')), config.scriptLoadTimeout);
        });
      ...
      try {
        ...
        return await Promise.race([
          new Promise((resolve, reject) => {
            // Set up global enforcement callback
            (window as any).useArkoseSetupEnforcement = async (enforcement: any) => {
              try {
                enforcement.setConfig({
                  ...config.parameters,
                  [config.selectorKey]: '#challenge',
                  [config.onErrorKey]: (error: any) => {
                    reject(ai.arkose._createError('ARKOSE_ERROR', error));
                  },
                  [config.onCompletedKey]: (result: any) => {
                    resolve(result[config.resultTokenKey]);
                  }
                });
                
                ai.arkose._arkose = enforcement;
                
                // Use first-time flag for run() vs reset()
                const method = ai.arkose._firstTimeFetchToken ? 'run' : 'reset';
                enforcement[method]();
              } catch (error) {
                reject(ai.arkose._createError('ENFORCEMENT_SETUP_FAILED', error));
              }
            };
            
            // If enforcement already exists, use it directly
            if (ai.arkose._arkose) {
              const method = ai.arkose._firstTimeFetchToken ? 'run' : 'reset';
              ai.arkose._arkose[method]();
            }
          }),
