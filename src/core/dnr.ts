...
interface IHTOSStorage {
  init(): Promise<void>;
  getToken(provider: string): Promise<{ token: string; expires?: number } | null>;
}

class MockHTOSStorage implements IHTOSStorage {
  async init(): Promise<void> {
    // Mock implementation
  }
  
  async getToken(provider: string): Promise<{ token: string; expires?: number } | null> {
...
export class DNRManager {
  private storage: IHTOSStorage;
  private lastRuleId: number = 1000;
  private rules: TrackedRule[] = [];
   ...
   */
  private onApiRequest(details: chrome.webRequest.WebRequestDetails): void {
   ...
   */
  private async registerRules(provider: string, tabId: number): Promise<void> {
    try {
   ...
   */
  private async cleanupTabRules(tabId: number): Promise<void> {
    const rulesToRemove = this.activeRules[tabId];
    if (!rulesToRemove) return;
   ...
   */
  async activateRulesForTab(tabId: number, provider: string): Promise<void> {
   ...
   */
  async deactivateRulesForTab(tabId: number): Promise<void> {
   ...
   * Remove rules by IDs
   */
  async unregisterByIds(ruleIds: number[]): Promise<void> {
   ...
   */
  private async dropAllSessionRules(): Promise<void> {
