...
export interface ITokenStorage {
  init(): Promise<void>;
  getToken(provider: string): Promise<ITokenData | null>;
  setToken(provider: string, tokenData: Omit<ITokenData, 'provider' | 'created' | 'lastUsed'>): Promise<void>;
  deleteToken(provider: string): Promise<void>;
  getAllTokens(): Promise<Record<string, ITokenData>>;
  clearExpiredTokens(): Promise<void>;
  isExpired(tokenData: ITokenData): boolean;
}
 ...
 */
export class SecureTokenStore implements ITokenStorage {
  private db: IDBDatabase | null = null;
  private readonly dbName = 'HTOSTokenStore';
   ...
   */
  async getToken(provider: string): Promise<ITokenData | null> {
   ...
   */
  async setToken(provider: string, tokenData: Omit<ITokenData, 'provider' | 'created' | 'lastUsed'>): Promise<void> {
    if (!this.db || !this.encryptionKey) {
      throw new Error('[HTOS] Storage not initialized');
    }

    try {
      const fullTokenData: ITokenData = {
        ...tokenData,
        provider,
        created: Date.now(),
        lastUsed: Date.now()
   ...
   */
  async getAllTokens(): Promise<Record<string, ITokenData>> {
    if (!this.db || !this.encryptionKey) {
      throw new Error('[HTOS] Storage not initialized');
    }
    ...
    try {
      ...
      const tokens: Record<string, ITokenData> = {};
    ...
    try {
      ...
      if (keyData.htos_encryption_key) {
        ...
        this.encryptionKey = await crypto.subtle.importKey(
          'raw',
          new Uint8Array(keyData.htos_encryption_key),
          { name: 'AES-GCM' },
          false,
          ['encrypt', 'decrypt']
        );
   ...
   */
  private async encryptTokenData(data: ITokenData): Promise<ArrayBuffer> {
   ...
   */
  private async decryptTokenData(encryptedBuffer: ArrayBuffer): Promise<ITokenData> {
    if (!this.encryptionKey) {
      throw new Error('[HTOS] Encryption key not initialized');
    }
    ...
    try {
      ...
      return JSON.parse(dataString) as ITokenData;
    } catch (error) {
